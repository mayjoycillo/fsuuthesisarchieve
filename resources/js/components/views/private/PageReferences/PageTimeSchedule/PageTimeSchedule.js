...
export default function PageTimeSchedule() {
    ...
    const { data: dataSource, refetch: refetchSource } = GET(
        `api/ref_time_schedule?${new URLSearchParams(tableFilter)}`,
        "time_schedule_list"
    );
    ...
    const {
        ...
        loading: loadingDeleteTimeSchedule,
    } = DELETE(`api/ref_time_schedule`, "time_schedule_list");

    const handleDelete = (record) => {
        mutateDeleteTimeSchedule(record, {
            onSuccess: (res) => {
                console.log("res", res);
                if (res.success) {
